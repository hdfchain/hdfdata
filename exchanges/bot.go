...
type ExchangeBot struct {
	...
	// BtcIndex is the (typically fiat) currency to which the HDF price should be
	...
	// exchanges or no up-to-date Hdfchain exchanges. IsFailed is a getter for failed.
	failed bool
}

// ExchangeBotState is the current known state of all exchanges, in a certain
// base currency, and a volume-averaged price and total volume in HDF.
...
func NewExchangeBot(config *ExchangeBotConfig) (*ExchangeBot, error) {
	...
	if len(bot.DcrBtcExchanges) == 0 {
		return nil, fmt.Errorf("no HDF-BTC exchanges were initialized")
	}
...
// updateExchange processes an update from a Hdfchain-BTC Exchange.
...
// sources or no Hdfchain Exchanges are up-to-date. Individual exchanges can
// be outdated/failed without IsFailed being false, as long as there is at least
// one Bitcoin index and one Hdfchain exchange.
...
// Conversion is a representation of some amount of HDF in another index.
...
func (bot *ExchangeBot) QuickSticks(token string, rawBin string) ([]byte, error) {
	...
	if !found {
		return nil, fmt.Errorf("Failed to find HDF exchange state for %s", token)
	}
