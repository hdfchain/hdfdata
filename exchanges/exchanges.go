...
import (
	...
	"github.com/hdfchain/hdfdata/dcrrates"
)
...
var (
	...
	BittrexURLs = URLs{
		Price: "https://bittrex.com/api/v1.1/public/getmarketsummary?market=btc-hdf",
		// Bittrex gives no documentation on the amount of data that will be
		// returned, and has no parameters to get more data.
		Depth: "https://bittrex.com/api/v1.1/public/getorderbook?market=btc-hdf&type=both",
		...
		Candlesticks: map[candlestickKey]string{
			hourKey: "https://bittrex.com/api/v2.0/pub/market/GetTicks?marketName=BTC-HDF&tickInterval=hour",
			dayKey:  "https://bittrex.com/api/v2.0/pub/market/GetTicks?marketName=BTC-HDF&tickInterval=day",
		},
...
// DcrExchanges maps tokens to constructors for HDF-BTC exchanges.
...
// opposed to a Hdfchain-to-Bitcoin Exchange.
func IsBtcIndex(token string) bool {
	_, ok := BtcIndices[token]
	return ok
}

// IsDcrExchange checks whether the given token is a known Hdfchain-BTC exchange.
...
// guaranteed is a price. For Hdfchain exchanges, the volumes will also be
// populated.
...
type HuobiCandlestickPt struct {
	...
	Vol    float64 `json:"vol"`    // Volume HDF
	Count  int64   `json:"count"`
}
