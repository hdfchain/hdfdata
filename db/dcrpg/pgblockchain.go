...
// Copyright (c) 2017, The hdfdata developers
...
// DB. Side chains known to hdfd are listed via the getchaintips RPC. Each block
...
// NewChainDB constructs a ChainDB for the given connection and Hdfchain network
...
// connection and Hdfchain network parameters. By default, duplicate row checks on
...
func NewChainDBWithCancel(ctx context.Context, cfg *ChainDBCfg, stakeDB *stakedb.StakeDatabase,
	...
	if !cockroach {
		...
		if err != nil {
			...
			// GRANT UPDATE ON TABLE hdfdata_mainnet.crdb_internal.zones TO hdfdata_user;
	...
	if !doLegacyUpgrade {
		...
		// table to match the blocks table, allowing hdfdata to start.
...
// DropTables drops (deletes) all of the known hdfdata tables.
...
func (pgb *ChainDB) AddressHistory(address string, N, offset int64,
	...
	log.Infof("%s: %d spent totalling %f HDF, %d unspent totalling %f HDF",
...
SPENDING_TX_DUPLICATE_CHECK:
	for _, f := range addressUTXOs.PrevOuts {
		...
		// outpoint because vin:i valuein is not reliable from hdfd.
...
// on the stake version with which hdfdata is compiled with (chaincfg.Params),
...
// GetVoteVersionInfo requests stake version info from the hdfd RPC server
...
func (pgb *ChainDB) GetAllTxOut(txid *chainhash.Hash) []*apitypes.TxOut {
	...
	for i := range txouts {
		...
		// types may need to be updated to match hdfd.
		if spk.Type != "invalid" && !apitypes.IsValidScriptClass(spk.Type) {
			log.Warnf(`The ScriptPubKey's type "%s" is not known to hdfdata! ` +
				`Update apitypes or debug hdfd.`)
		}
...
// GetSDiff gets the stake difficulty in HDF for a given block height.
...
// GetSBitsByHash gets the stake difficulty in HDF for a given block height.
...
// GetSDiffRange gets the stake difficulties in HDF for a range of block heights.
...
func (pgb *ChainDB) GetExplorerBlock(hash string) *exptypes.BlockInfo {
	...
	for _, tx := range data.RawSTx {
		...
		case stake.TxTypeSSGen:
			...
			// well.
			if stx.Fee > 0 {
				log.Tracef("Vote with fee: %d atoms, %.8f HDF", int64(stx.Fee), stx.Fees)
			}
...
// CurrentDifficulty returns the current difficulty from hdfd.
