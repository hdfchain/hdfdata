...
// by hdfdata or rebuilddb2. The patch versions may also be different. They
...
// CreateTablesLegacy creates all tables required by hdfdata if they do not
// already exist.
...
// style is run after updating hdfdata past version 3.0.0.
...
// lnfeatures agenda ids exist since appropriate voting mechanisms on the hdfd
...
func (pgb *ChainDB) UpgradeTables(hdfdClient BlockGetter, version, needVersion TableVersion) (bool, error) {
	...
	case version.major == 3 && version.minor == 1 && version.patch == 0:
		...
		isSuccess, er := pgb.initiatePgUpgrade(hdfdClient, theseUpgrades)
		if !isSuccess {
			return isSuccess, er
		}
	...
	case version.major == 3 && version.minor == 5 && version.patch == 4:
		toVersion = TableVersion{3, 5, 5} // hdfdata 3.1 release
	...
	case version.major == 3 && version.minor == 9 && version.patch == 0:
		...
		rawChainInfo, err := hdfdClient.GetBlockChainInfo()
		if err != nil {
			return false, err
		}
...
func verifyChainWork(client BlockGetter, db *sql.DB) (int64, error) {
	...
	for rows.Next() {
		...
		if err != nil {
			// If it's an orphaned block, it may not be in hdfd database. OK to skip.
			if strings.HasPrefix(err.Error(), "-5: Block not found") {
				if isMainchain {
					// Although every mainchain block should have a corresponding
					// blockNode and chainwork value in hdfd, this upgrade is run before
