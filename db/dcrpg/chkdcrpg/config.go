...
var (
	...
	hdfdataHomeDir = dcrutil.AppDataDir("hdfdata", false)
	hdfdataDataDir = filepath.Join(hdfdataHomeDir, defaultDataDirName)
	...
	defaultDBUser     = "hdfdata"
	defaultDBPass     = ""
	defaultDBName     = "hdfdata"
)
...
type config struct {
	...
	DcrdataDataDirectory string `long:"hdfdata-datadir" description:"Path to a hdfdata datadir" env:"HDFDATA_DATA_DIR"`
	...
	// RPC client options
	HdfdUser         string `long:"hdfduser" description:"Daemon RPC user name"`
	HdfdPass         string `long:"hdfdpass" description:"Daemon RPC password"`
	Hdfdserv         string `long:"hdfdserv" description:"Hostname/IP and port of hdfd RPC server to connect to (default localhost:9109, testnet: localhost:19109, simnet: localhost:19556)"`
	HdfdCert         string `long:"hdfdcert" description:"File containing the hdfd certificate file"`
...
func loadConfig() (*config, error) {
	...
	if cfg.DcrdataDataDirectory == "" {
		cfg.DcrdataDataDirectory = hdfdataDataDir
	}
	...
	// them.
	if cfg.Hdfdserv == "" {
		cfg.Hdfdserv = defaultDcrdHost + ":" + activeNet.JSONRPCClientPort
	}
