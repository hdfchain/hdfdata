...
// hdfd does not supply vote counts for completed votes, so the tracker will
...
// Since version could technically be updated without turning off hdfdata,
...
func (tracker *VoteTracker) refreshRCI() (*chainjson.GetVoteInfoResult, error) {
	...
	if voteInfo == nil {
		if oldVersion == 0 {
			...
			log.Info("No agenda information retrieved from hdfd.")
			return nil, nil
		}
...
// Once all resources have been retrieved from hdfd, update VoteTracker fields.
