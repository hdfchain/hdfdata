...
import (
	...
	"github.com/hdfchain/hdfdata/gov/v3/politeia/types"
...
func TestNewProposalsDB(t *testing.T) {
	var count int
	inputURLPath := "https://proposals.clkj.ltd"
	expectedPath := "https://proposals.clkj.ltd/api/v1"
...
func TestGenerateCustomID(t *testing.T) {
	...
	td := []testData{
		...
		{
			title:    "Hdfchain Bug Bounty Proposal",
			customID: "hdfchain-bug-bounty-proposal",
		},
		{
		...
		{
			title:    "Proposed Statement Of Work (SOW) For Hdfchain Blockchain Wallet Tutorial Campaign",
			customID: "proposed-statement-of-work-sow-for-hdfchain-blockchain-wallet-tutorial-campaign",
		},
	}
...
func TestSaveProposals(t *testing.T) {
	...
	t.Run("Test_update_proposal_if_same_tokenID_and_new_RefID_found", func(t *testing.T) {
		...
		copy2FirstProposal.Name = "Integrate hdfchain with Trezor hardware wallet."
	...
	t.Run("Test_create_new_proposal_if_new_tokenID_and_duplicate_RefID_found", func(t *testing.T) {
		...
		copy2MockProposal.Name = "Integrate hdfchain with Trezor hardware wallet."
