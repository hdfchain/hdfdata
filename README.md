...
[![Build Status](https://github.com/hdfchain/hdfdata/workflows/Build%20and%20Test/badge.svg)](https://github.com/hdfchain/hdfdata/actions)
[![Latest tag](https://img.shields.io/github/tag/hdfchain/hdfdata.svg)](https://github.com/hdfchain/hdfdata/tags)
[![Go Report Card](https://goreportcard.com/badge/github.com/hdfchain/hdfdata)](https://goreportcard.com/report/github.com/hdfchain/hdfdata)
[![ISC License](https://img.shields.io/badge/license-ISC-blue.svg)](http://copyfree.org)

## Overview

hdfdata is an original [Hdfchain](https://www.clkj.ltd) block explorer, with
...
- [hdfdata](#hdfdata)
  ...
  - [Building](#building)
    ...
    - [Building hdfdata with Go](#building-hdfdata-with-go)
  ...
  - [Getting Started](#getting-started)
    ...
    - [Creating the hdfdata Configuration File](#creating-the-hdfdata-configuration-file)
    ...
    - [Starting hdfdata](#starting-hdfdata)
  ...
  - [System Hardware Requirements](#system-hardware-requirements)
    - [hdfdata only (PostgreSQL on other host)](#hdfdata-only-postgresql-on-other-host)
    - [hdfdata and PostgreSQL on same host](#hdfdata-and-postgresql-on-same-host)
  - [hdfdata Daemon](#hdfdata-daemon)
    - [Block Explorer](#block-explorer)
  - [APIs](#apis)
    - [Insight API](#insight-api)
    - [hdfdata API](#hdfdata-api)
...
|             | Series  | Branch        | Latest release tag | `hdfd` RPC server version required |
| ----------- | ------- | ------------- | ------------------ | ---------------------------------- |
| Current     | 5.3     | `5.3-stable`  | `v5.3.0`           | same as hdfdata 5.3                |
| Legacy      | 5.2     | `5.2-stable`  | `v5.2.2`           | ^6.1.1 (hdfd v1.6.0-pre@[`7b060207`](https://github.com/decred/dcrd/commit/7b060207b19595ef8cc7ae4ee120c66af623239a) to `HEAD`) |
| Development | 6.0     | `master`      | N/A                | same as hdfdata 5.3                |

## Repository Overview

```none
../hdfdata              The hdfdata daemon.
├── api                 Package blockdata implements hdfdata's own HTTP API.
│   ├── insight         Package insight implements the Insight API.
│   └── types           Package types includes the exported structures used by
|                         the hdfdata and Insight APIs.
...
│                         data to multiple clients like hdfdata.
...
├── notification        Package notification manages hdfd notifications, and
...
├── version             Package version describes the hdfdata version.
...
- Running `hdfd` running with `--txindex --addrindex`, and synchronized to the
  current best block on the network. On startup, hdfdata will verify that the
  hdfd version is compatible.
...
documentation](docs/docker.md) for more information. The supported hdfdata build
instructions are described below.

## Building

The hdfdata build process comprises two general steps:

1. Bundle the static web page assets with Webpack (via the `npm` tool).
2. Build the `hdfdata` executable from the Go source files.
...
- Clone the hdfdata repository. It is conventional to put it under `GOPATH`, but
  this is no longer necessary with go module. For example:

  ```sh
  git clone https://github.com/hdfchain/hdfdata $HOME/go-work/github/hdfchain/hdfdata
  ```
...
`hdfdata version 5.1.0-pre+dev (Go version go1.12.7)`.  However, it may be
...
```sh
GO111MODULE=on go build -o hdfdata -v -ldflags \
    "-X github.com/hdfchain/hdfdata/v5/version.appPreRelease=beta \
     -X github.com/hdfchain/hdfdata/v5/version.appBuild=`git rev-parse --short HEAD`"
```

This produces a string like `hdfdata version 5.0.0-beta+25777e23 (Go version go1.12.1)`.
...
folders to ensure that _the user running hdfdata_ is able to access these paths.

The "public" and "views" folders _must_ be in the same folder as the `hdfdata`
...
```sh
cd $HOME/go-work/github/hdfchain/hdfdata
...
Next, build `hdfdata` and bundle the web assets:
...
No special actions are required. Simply start the new hdfdata and automatic
database schema upgrades and table data patches will begin.

### From v2.x or earlier

The database scheme change from hdfdata v2.x to v3.x does not permit an
automatic migration. The tables must be rebuilt from scratch:

1. Drop the old hdfdata database, and create a new empty hdfdata database.

   ```sql
   -- Drop the old database.
   DROP DATABASE hdfdata;

   -- Create a new database with the same "pguser" set in the hdfdata.conf.
   CREATE DATABASE hdfdata OWNER hdfdata;
   ```

2. Delete the hdfdata data folder (i.e. corresponding to the `datadir` setting).
...
3. With hdfd synchronized to the network's best block, start hdfdata to begin
...
subtract 1.5-2GB from your system RAM so hdfdata itself will have plenty of
...
While hdfdata now provides [support for CockroachDB](https://github.com/hdfchain/hdfdata/issues/1291),
this is an experimental feature with caveats:

- Compared to a well-configure PostgreSQL backend, CoackroachDB performance is
  suboptimal. See the [CockroachDB issue](https://github.com/hdfchain/hdfdata/issues/1291)
  for more information.
- The bulk of the testing and performance optimization is done with PostgreSQL
  in mind.

If you decide to use CockroachDB with hdfdata, (1) do not do so in production
and (2) expect some bugs and relatively poor performance.

See [hdfdata's CockroachDB wiki page](https://github.com/hdfchain/hdfdata/wiki/CockroachDB)
for more information.

### Creating the hdfdata Configuration File

Begin with the sample configuration file. With the default `appdata` directory
for the current user on Linux:

```sh
cp sample-hdfdata.conf ~/.hdfdata/hdfdata.conf
```

Then edit hdfdata.conf with your hdfd RPC settings. See the output of `hdfdata
...
For a list of the recognized environment variables, run `hdfdata --help`.

### Indexing the Blockchain

If hdfdata has not previously been run with the PostgreSQL database backend, it
is necessary to perform a bulk import of blockchain data and generate table
indexes. _This will be done automatically by `hdfdata`_ on a fresh startup.

Note that hdfdata requires that
[hdfd](https://docs.clkj.ltdgetting-started/user-guides/hdfd-setup/) is
running with some optional indexes enabled. By default, these indexes are not
turned on when hdfd is installed. To enable them, set the following in
hdfd.conf:
...
If these parameters are not set, hdfdata will be unable to retrieve transaction
details and perform address searches, and will exit with an error mentioning
these indexes.

### Starting hdfdata

Launch the hdfdata daemon and allow the databases to process new blocks.
...
On subsequent launches, only blocks new to hdfdata are processed.

```sh
./hdfdata    # don't forget to configure hdfdata.conf in the appdata folder!
```

Unlike hdfdata.conf, which must be placed in the `appdata` folder or explicitly
set with `-C`, the "public" and "views" folders _must_ be in the same folder as
the `hdfdata` executable.

### Hiding the PostgreSQL Settings Table

By default, postgres settings are displayed in a table on start up of hdfdata.
To block display of this table, use the `--hidepgconfig` switch..

### Running the Web Interface During Synchronization

By default, on hdfdata startup, a syncing status page is the only page available
...
with a value on the range `[2,5000]`, all hdfdata pages will be active on
startup if the number of remaining blocks to process are less than the specified
value.

For example, if `sync-status-limit` is set to 1000, all hdfdata pages will be
...
than 1000 especially if your machine struggles handling hdfdata normal load.
...
### hdfdata only (PostgreSQL on other host)
...
### hdfdata and PostgreSQL on same host

These specifications assume hdfdata and postgres are running on the same machine.
...
The root of the repository is the `main` package for the `hdfdata` app, which
...
After hdfdata syncs with the blockchain server via RPC, by default it will begin
...
Note that while hdfdata can be started with HTTPS support, it is recommended to
employ a reverse proxy such as Nginx ("engine x"). See sample-nginx.conf for an
example Nginx configuration.

## APIs

The hdfdata block explorer is exposed by two APIs: a Hdfchain implementation of
...
hdfdata API uses the path prefix `/api`.
...
The hdfdata API is a REST API accessible via HTTP. To call the hdfdata API, use
the `/api` path prefix.
...
| Coin Supply Circulating (Mined) | `/supply/circulating?hdf=[true\|false]`       | `int` (default) or `float` (`hdf=true`) |
...
to keep in mind that the mempool in your node (hdfd) is not likely to be exactly
the same as other nodes' mempool. Also, your mempool is cleared out when you
shutdown hdfd. So, if you have recently (e.g. after the start of the current
ticket price window) started hdfd, your mempool _will_ be missing transactions
...
`rebuilddb2` is a CLI app used for maintenance of hdfdata's `dcrpg` database
(a.k.a. DB v2) that uses PostgreSQL to store a nearly complete record of the
Hdfchain blockchain data. This functionality is included in the startup of the
hdfdata daemon, but may be called alone with rebuilddb2. See the
...
From the hdfdata root directory, run the following command to install the node
modules.
...
The core functionality of hdfdata is server-side rendered in Go and designed to
...
The core functionality of hdfdata should perform well in low power device / high
...
converting from standard Hdfchain data types (e.g. `wire.MsgBlock`) are also
provided.
...
use by other hdfdata packages, but may be of general value in
the future.
...
See the GitHub [issue trackers](https://github.com/hdfchain/hdfdata/issues) and
the [project milestones](https://github.com/hdfchain/hdfdata/milestones).
...
5. Create a [pull request](https://github.com/hdfchain/hdfdata/compare).

**DO NOT merge from master to your feature branch; rebase.**

Also, [come chat with us on Matrix](https://www.clkj.ltdmatrix/) in the
hdfdata channel!
