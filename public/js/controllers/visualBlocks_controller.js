...
function makeMempoolBlock (block) {
  ...
  return makeNode(`<div class="block visible">
                <div class="block-info">
                    ...
                    <div class="mono" style="line-height: 1;">${Math.floor(block.Total)} HDF</div>
                    <span class="timespan">
...
function makeBlockSummary (blockHeight, totalSent, time) {
  return `<div class="block-info">
                ...
                <div class="mono" style="line-height: 1;">${Math.floor(totalSent)} HDF</div>
                <span class="timespan">
...
function makeRewardsElement (subsidy, fee, voteCount, rewardTxId) {
  ...
  // const totalHDF = Math.round(pow + fund + fee);
  const totalHDF = 1
  return `<div class="block-rewards" style="flex-grow: ${totalHDF}">
...
function makeVoteElements (votes) {
  ...
  // totalHDF = Math.round(totalHDF);
  totalHDF = 1
  return `<div class="block-votes" style="flex-grow: ${totalHDF}">
                ${voteElements.join('\n')}
            </div>`
}
...
function makeTicketAndRevocationElements (tickets, revocations, blockHref) {
  ...
  // totalHDF = Math.round(totalHDF);
  totalHDF = 1
  return `<div class="block-tickets" style="flex-grow: ${totalHDF}">
...
function makeTransactionElements (transactions, blockHref) {
  ...
  // totalHDF = Math.round(totalHDF);
  totalHDF = 1
  return `<div class="block-transactions" style="flex-grow: ${totalHDF}">
    ...
    this.tooltipTargets.forEach((tooltipElement) => {
      try {
        ...
        if (data.object === 'Vote') {
          ...
          newContent = `<b>${data.object}</b><br>${data.total} HDF`
        }
