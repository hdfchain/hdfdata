...
function anonymitySetFunc (data) {
  ...
  if (data.axis === 'height') {
    ...
    d = data.t.map((t, i) => {
      ...
      return [new Date(t * 1000), data.anonymitySet[i] * atomsToHDF]
    })
  }
...
function ticketPriceFunc (data) {
  if (data.t) return zipWindowTvYZ(data.t, data.price, data.count, atomsToHDF)
  return zipWindowHvYZ(data.price, data.count, data.window, atomsToHDF)
}
...
function percentStakedFunc (data) {
  rawCoinSupply = data.circulation.map(v => v * atomsToHDF)
  rawPoolValue = data.poolval.map(v => v * atomsToHDF)
...
function circulationFunc (chartData) {
  ...
  var data = map(supplies, (n, i) => {
    ...
    addDough(height)
    inflation.push(yMax)
    return [xFunc(i), supplies[i] * atomsToHDF, null, anonymitySet[i] * atomsToHDF]
  })
  ...
  plotGraph (chartName, data) {
    ...
    switch (chartName) {
      case 'ticket-price': // price graph
        ...
        yFormatter = customYFormatter(y => y.toFixed(8) + ' HDF')
        break
      ...
      case 'stake-participation':
        ...
        yFormatter = (div, data, i) => {
          ...
          div.appendChild(legendEntry(`${legendMarker()} Ticket Pool Value: ${intComma(rawPoolValue[i])} HDF`))
          div.appendChild(legendEntry(`${legendMarker()} Coin Supply: ${intComma(rawCoinSupply[i])} HDF`))
        }
        break

      case 'ticket-pool-value': // pool value graph
        d = zip2D(data, data.poolval, atomsToHDF)
        assign(gOptions, mapDygraphOptions(d, [xlabel, 'Ticket Pool Value'], true,
          'Ticket Pool Value (HDF)', true, false))
        yFormatter = customYFormatter(y => intComma(y) + ' HDF')
        break
      ...
      case 'coin-supply': // supply graph
        ...
        assign(gOptions, mapDygraphOptions(d.data, [xlabel, 'Coin Supply', 'Inflation Limit', 'Mix Rate'],
          true, 'Coin Supply (HDF)', true, false))
        ...
        yFormatter = (div, data, i) => {
          addLegendEntryFmt(div, data.series[0], y => intComma(y) + ' HDF')
          var change = 0
          if (i < d.inflation.length) {
            ...
            if (this.anonymitySetTarget.checked) {
              ...
              div.appendChild(legendEntry(`${legendMarker()} Mixed: ${intComma(mixed)} HDF (${mixedPercentage}%)`))
            }
            let predicted = d.inflation[i]
            let unminted = predicted - data.series[0].y
            change = ((unminted / predicted) * 100).toFixed(2)
            div.appendChild(legendEntry(`${legendMarker()} Unminted: ${intComma(unminted)} HDF (${change}%)`))
          }
        }
        break

      case 'fees': // block fee graph
        d = zip2D(data, data.fees, atomsToHDF)
        assign(gOptions, mapDygraphOptions(d, [xlabel, 'Total Fee'], false, 'Total Fee (HDF)', true, false))
        break
      ...
      case 'privacy-participation': // anonymity set graph
        ...
        assign(gOptions, mapDygraphOptions(d.data, [xlabel, label], false, `${label} (HDF)`, true, false))

        yFormatter = (div, data, i) => {
          addLegendEntryFmt(div, data.series[0], y => y > 0 ? intComma(y) : '0' + ' HDF')
        }
        break
