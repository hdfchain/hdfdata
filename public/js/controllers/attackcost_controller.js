...
var height, hdfPrice, hashrate, tpSize, tpValue, tpPrice, graphData, currentPoint, coinSupply
...
function rateCalculation (y) {
  ...
  // https://medium.com/hdfchain/decreds-hybrid-protocol-a-superior-deterrent-to-majority-attacks-9421bf486292
...
const deviceList = {
  ...
  '1': {
    ...
    cost: 4199, // $
    name: 'D1',
    link: 'https://www.cryptocompare.com/mining/crypto-drilling/microbt-whatsminer-d1-plus-psu-hdf-44ths/'
  }
}
...
export default class extends Controller {
  static get targets () {
    return [
      ...
      'kwhRateLabel', 'otherCosts', 'otherCostsValue', 'priceHDF', 'internalAttackText', 'targetHashRate',
      ...
      'total', 'totalHDFPos', 'totalDeviceCost', 'totalElectricity', 'totalExtraCostRate', 'totalKwh', 'totalPos', 'totalPow',
      'graph', 'labels', 'projectedTicketPrice', 'projectedTicketPriceIncrease', 'attackType', 'attackPosPercentAmountLabel',
      'hdfPriceLabel', 'totalHDFPosLabel', 'projectedPriceDiv', 'attackNotPossibleWrapperDiv', 'coinSupply', 'totalAttackCostContainer'
    ]
  }
  ...
  async connect () {
    ...
    hdfPrice = parseFloat(this.data.get('hdfprice'))
    ...
    if (this.settings.price) this.priceHDFTarget.value = parseFloat(this.settings.price)
  ...
  updatePrice () {
    this.settings.price = this.priceHDFTarget.value
    hdfPrice = this.priceHDFTarget.value
    this.updateSliderData()
  }
  ...
  calculate (disableHashRateUpdate) {
    ...
    var ticketAttackSize, HDFNeed
    if (this.settings.attack_type === externalAttackType) {
      HDFNeed = tpValue / (1 - parseFloat(this.targetPosTarget.value) / 100)
      this.setAllValues(this.newTicketPoolValueTargets, digitformat(HDFNeed, 2))
      this.setAllValues(this.additionalDcrTargets, digitformat(HDFNeed - tpValue, 2))
    } else {
      ticketAttackSize = (tpSize * parseFloat(this.targetPosTarget.value)) / 100
      HDFNeed = tpValue * (parseFloat(this.targetPosTarget.value) / 100)
      this.setAllValues(this.ticketPoolAttackTargets, digitformat(HDFNeed))
    }
    var projectedTicketPrice = HDFNeed / tpSize
    ...
    var totalHDFPos = this.settings.attack_type === externalAttackType ? HDFNeed - tpValue : ticketAttackSize * projectedTicketPrice
    var totalPos = totalHDFPos * hdfPrice
    ...
    this.priceHDFTarget.value = digitformat(hdfPrice, 2)
    ...
    this.setAllValues(this.totalHDFPosTargets, digitformat(totalHDFPos, 2))
    ...
    this.setAllValues(this.totalHDFPosLabelTargets, digitformat(totalHDFPos, 2))
    this.setAllValues(this.hdfPriceLabelTargets, digitformat(hdfPrice, 2))
    this.showPosCostWarning(HDFNeed)
  }
  ...
  showPosCostWarning (HDFNeed) {
    var totalHDFInCirculation = coinSupply / 100000000
    if (HDFNeed > totalHDFInCirculation) {
      this.coinSupplyTarget.textContent = digitformat(totalHDFInCirculation, 2)
