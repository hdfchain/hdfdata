...
// Package notification synchronizes hdfd notifications to any number of
// handlers. Typical use:
// 1. Create a Notifier with NewNotifier.
// 2. Grab hdfd configuration settings with DcrdHandlers.
// 3. Create an hdfd/rpcclient.Client with the settings from step 2.
...
//    providing as an argument the hdfd client created in step 3.
...
// TxHandler is a function that will be called when hdfd reports new mempool
// transactions.
type TxHandler func(*chainjson.TxRawResult) error

// BlockHandler is a function that will be called when hdfd reports a new block.
type BlockHandler func(*wire.BlockHeader) error

// BlockHandlerLite is a simpler trigger using only bultin types, also called
// when hdfd reports a new block.
type BlockHandlerLite func(uint32, string) error

// ReorgHandler is a function that will be called when hdfd reports a reorg.
type ReorgHandler func(*txhelpers.ReorgData) error

// Notifier handles block, tx, and reorg notifications from a hdfd node. Handler
// functions are registered with the Register*Handlers methods. To start the
// Notifier, Listen must be called with a hdfd rpcclient.Client only after all
// handlers are registered.
type Notifier struct {
	ctx  context.Context
	node HDFDNode
	// The anyQ sequences all hdfd notification in the order they are received.
	anyQ     chan interface{}
	tx       [][]TxHandler
	block    [][]BlockHandler
	reorg    [][]ReorgHandler
...
// HDFDNode is an interface to wrap a hdfd rpcclient.Client. The interface
// allows testing with a dummy node.
type HDFDNode interface {
...
// DcrdHandlers creates a set of handlers to be passed to the hdfd
...
func (notifier *Notifier) DcrdHandlers() *rpcclient.NotificationHandlers {
	return &rpcclient.NotificationHandlers{
		...
		// for the mempool monitors to avoid an extra call to hdfd for
		// the tx details
		OnTxAcceptedVerbose: notifier.onTxAcceptedVerbose,
	}
}

// superQueue should be run as a goroutine. The hdfd-registered block and reorg
...
// packages don't necessarily need to import hdfd/wire. Handlers registered with
...
// signalReorg takes the basic reorganization data from hdfd, determines the two
