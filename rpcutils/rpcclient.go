...
// Any of the following hdfd RPC API versions are deemed compatible with
// hdfdata.
...
// ConnectNodeRPC attempts to create a new websocket connection to a hdfd node,
...
func ConnectNodeRPC(host, user, pass, cert string, disableTLS, disableReconnect bool,
	...
	if !disableTLS {
		...
		if err != nil {
			log.Errorf("Failed to read hdfd cert file at %s: %s\n",
				cert, err.Error())
			return nil, nodeVer, err
		}
		log.Debugf("Attempting to connect to hdfd RPC %s as user %s "+
			"using certificate located in %s",
			host, user, cert)
	} else {
		log.Debugf("Attempting to connect to hdfd RPC %s as user %s (no TLS)",
			host, user)
	}
	...
	hdfdClient, err := rpcclient.New(connCfgDaemon, ntfnHdlrs)
	if err != nil {
		return nil, nodeVer, fmt.Errorf("Failed to start hdfd RPC client: %s", err.Error())
	}

	// Ensure the RPC server has a compatible API version.
	ver, err := hdfdClient.Version()
	if err != nil {
		log.Error("Unable to get RPC version: ", err)
		return nil, nodeVer, fmt.Errorf("unable to get node RPC version")
	}

	hdfdVer := ver["hdfdjsonrpcapi"]
	nodeVer = semver.NewSemver(hdfdVer.Major, hdfdVer.Minor, hdfdVer.Patch)

	// Check if the hdfd RPC API version is compatible with hdfdata.
...
// orphaned DB tip whose corresponding block is not known to hdfd will not cause
...
func OrphanedTipLength(ctx context.Context, client BlockHashGetter,
	...
	for {
		...
		if err != nil {
			return -1, fmt.Errorf("Unable to retrieve hdfd block at height %d: %v", commonHeight, err)
		}
		...
		// Reorgs are soft-limited to depth 6 by hdfd. More than six blocks without
