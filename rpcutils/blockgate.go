...
// directly from hdfd via RPC and subsequently update the internal block cache
...
// NodeHeight gets the chain height from hdfd.
...
// the event of a cache miss, the block is retrieved from hdfd via RPC.
func (g *BlockGate) Block(hash chainhash.Hash) (*dcrutil.Block, error) {
	...
	// Cache miss. Retrieve from hdfd RPC.
...
// hdfd, stores it in cache, and signals any waiters. This is the thread-safe
...
// updateToBlock gets the block at the specified height on the main chain from
// hdfd. It is not thread-safe. It wrapped by UpdateToBlock for thread-safety,
