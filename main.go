...
import (
	...
	"github.com/hdfchain/hdfdata/blockdata/v5"
	"github.com/hdfchain/hdfdata/db/cache/v3"
	"github.com/hdfchain/hdfdata/db/dbtypes/v2"
	"github.com/hdfchain/hdfdata/db/dcrpg/v5"
	"github.com/hdfchain/hdfdata/exchanges/v2"
	"github.com/hdfchain/hdfdata/gov/v3/agendas"
	"github.com/hdfchain/hdfdata/gov/v3/politeia"
	"github.com/hdfchain/hdfdata/mempool/v5"
	...
	"github.com/hdfchain/hdfdata/rpcutils/v3"
	"github.com/hdfchain/hdfdata/semver"
	"github.com/hdfchain/hdfdata/stakedb/v3"
	"github.com/hdfchain/hdfdata/v5/api"
	"github.com/hdfchain/hdfdata/v5/api/insight"
	"github.com/hdfchain/hdfdata/v5/explorer"
	notify "github.com/hdfchain/hdfdata/v5/notification"
...
func _main(ctx context.Context) error {
	...
	cfg, err := loadConfig()
	if err != nil {
		fmt.Printf("Failed to load hdfdata config: %s\n", err.Error())
		return err
	}
	defer func() {
		if logRotator != nil {
	...
	// Connect to hdfd RPC server using a websocket.
	hdfdClient, nodeVer, err := connectNodeRPC(cfg, notifier.DcrdHandlers())
	if err != nil || hdfdClient == nil {
		return fmt.Errorf("Connection to hdfd failed: %v", err)
	}

	defer func() {
		if hdfdClient != nil {
			log.Infof("Closing connection to hdfd.")
			hdfdClient.Shutdown()
			hdfdClient.WaitForShutdown()
		}
		log.Infof("Bye!")
	...
	curnet, err := hdfdClient.GetCurrentNet()
	if err != nil {
		return fmt.Errorf("Unable to get current network from hdfd: %v", err)
	}
	log.Infof("Connected to hdfd (JSON-RPC API v%s) on %v",
	...
	// StakeDatabase
	stakeDB, stakeDBHeight, err := stakedb.NewStakeDatabase(hdfdClient, activeChain, cfg.DataDir)
	if err != nil {
		log.Errorf("Unable to create stake DB: %v", err)
		if stakeDBHeight >= 0 {
			log.Infof("Attempting to recover stake DB...")
			stakeDB, err = stakedb.LoadAndRecover(hdfdClient, activeChain, cfg.DataDir, stakeDBHeight-288)
	...
	mpChecker := rpcutils.NewMempoolAddressChecker(hdfdClient, activeChain)
	chainDB, err := dcrpg.NewChainDBWithCancel(ctx, &dbCfg,
		stakeDB, mpChecker, piParser, hdfdClient, requestShutdown)
	if chainDB != nil {
		defer chainDB.Close()
	}
	if err != nil {
	...
	Heights := func() (nodeHeight, chainDBHeight int64, err error) {
		_, nodeHeight, err = hdfdClient.GetBestBlock()
		if err != nil {
	...
	if auxHeight > -1 {
		orphaned, err := rpcutils.OrphanedTipLength(ctx, hdfdClient, auxHeight, chainDB.BlockHash)
		if err != nil {
	...
	blockHash, nodeHeight, err := hdfdClient.GetBestBlock()
	if err != nil {
		return fmt.Errorf("Unable to get block from node: %v", err)
	}

	block, err := hdfdClient.GetBlockHeader(blockHash)
	if err != nil {
		return fmt.Errorf("unable to fetch the block from the node: %v", err)
	}

	// bestBlockAge is the time since the hdfd best block was mined.
	...
	// expected height of the best block from hdfd now should be this.
	...
	collector := blockdata.NewCollector(hdfdClient, activeChain, stakeDB)
	if collector == nil {
	...
	// transactions that appear in the hdfchain blockchain. If corrupted data is
	...
	agendaDB, err = agendas.NewAgendasDB(
		hdfdClient, filepath.Join(cfg.DataDir, cfg.AgendasDBFileName))
	if err != nil {
	...
	// data stored in github repositories away from the hdfchain blockchain. It also
	...
	if !cfg.SimNet {
		tracker, err = agendas.NewVoteTracker(activeChain, hdfdClient,
	...
	mpoolCollector := mempool.NewMempoolDataCollector(hdfdClient, activeChain)
	...
	mpm, err := mempool.NewMempoolMonitor(ctx, mpoolCollector, mempoolSavers,
		activeChain, hdfdClient, mempoolSigOuts, true)
	...
	if displaySyncStatusPage {
		...
		latestDBBlockHash, err := hdfdClient.GetBlockHash(chainDBHeight)
		if err != nil {
	...
	insightSocketServer, err := insight.NewSocketServer(activeChain, hdfdClient)
	if err != nil {
	...
	webMux.With(explore.SyncStatusAPIIntercept).Group(func(r chi.Router) {
		// Mount the hdfdata's REST API.
		r.Mount("/api", apiMux.Mux)
		// Setup and mount the Insight API.
		insightApp := insight.NewInsightApi(hdfdClient, chainDB,
	...
	getSyncd := func(updateAddys, newPGInds bool) (int64, error) {
		...
		if cfg.NoBlockPrefetch {
			...
			pfc := rpcutils.NewBlockPrefetchClient(hdfdClient)
			defer func() {
				pfc.Stop()
	...
	ensureSync := func() error {
		...
		_, height, err := hdfdClient.GetBestBlock()
		if err != nil {
			return fmt.Errorf("unable to get block from node: %v", err)
		}
	...
	// Ensure all side chains known by hdfd are also present in the auxiliary DB
	...
	if cfg.ImportSideChains {
		...
		log.Info("Aux DB -> Retrieving side chain blocks from hdfd...")
	...
	// Monitors that fetch the latest updates from hdfd will be launched next.
	...
	// hdfd notifies of block 200000 while hdfdata has only reached 1000 in
	...
	// already registered for notifications with hdfd but the monitors below are
	// not started, notifications will fill up the channels, only to be
	// processed after sync. This is also incorrect since hdfd might notify of a
...
func connectNodeRPC(cfg *config, ntfnHandlers *rpcclient.NotificationHandlers) (*rpcclient.Client, semver.Semver, error) {
	return rpcutils.ConnectNodeRPC(cfg.Hdfdserv, cfg.HdfdUser, cfg.HdfdPass,
		cfg.HdfdCert, cfg.DisableDaemonTLS, true, ntfnHandlers)
}
...
func listenAndServeProto(ctx context.Context, wg *sync.WaitGroup, listen, proto string, mux http.Handler) {
	...
	go func() {
		var err error
		if proto == "https" {
			err = server.ListenAndServeTLS("hdfdata.cert", "hdfdata.key")
		} else {
			err = server.ListenAndServe()
		}
		// If the server dies for any reason other than ErrServerClosed (from
		// graceful server.Shutdown), log the error and request hdfdata be
		// shutdown.
