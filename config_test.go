...
func TestMain(m *testing.M) {
	...
	tempConfigFile, _ = ioutil.TempFile("", "hdfdata_test_file.cfg")
	defer os.Remove(tempConfigFile.Name())
	os.Setenv("HDFDATA_CONFIG_FILE", tempConfigFile.Name())

	// Make an empty folder for appdata tests.
	tempAppDataDir, _ = ioutil.TempDir("", "hdfdata_test_appdata")
	...
	retCode := m.Run()
	os.Unsetenv("HDFDATA_CONFIG_FILE")

	os.Exit(retCode)
}

// disableConfigFileEnv checks if the HDFDATA_CONFIG_FILE environment variable
// is set, unsets it, and returns a function that will return
// HDFDATA_CONFIG_FILE to its state before calling disableConfigFileEnv.
func disableConfigFileEnv() func() {
	loc, wasSet := os.LookupEnv("HDFDATA_CONFIG_FILE")
	if wasSet {
		os.Unsetenv("HDFDATA_CONFIG_FILE")
		return func() { os.Setenv("HDFDATA_CONFIG_FILE", loc) }
	}
	return func() {}
}
...
func TestLoadCustomConfigPresent(t *testing.T) {
	...
	_, err := loadConfig()
	if err != nil {
		t.Fatalf("Failed to load hdfdata config: %v", err)
	}
}
...
func TestLoadDefaultConfigMissing(t *testing.T) {
	...
	os.Setenv("HDFDATA_APPDATA_DIR", tempAppDataDir)
	defer os.Unsetenv("HDFDATA_APPDATA_DIR")
...
func TestLoadCustomConfigMissing(t *testing.T) {
	...
	os.Setenv("HDFDATA_CONFIG_FILE", goneFile.Name())

	// Attempt to load using the non-existent non-default config file, which
	// should return an error.
	_, err := loadConfig()
	if err == nil {
		t.Errorf("Loaded hdfdata config, but the explicitly set config file"+
...
func TestDefaultConfigAPIListenWithEnv(t *testing.T) {
	...
	os.Setenv("HDFDATA_LISTEN_URL", customListenPath)
...
func TestDefaultConfigAppDataDir(t *testing.T) {
	expected := dcrutil.AppDataDir("hdfdata", false)
	cfg, err := loadConfig()
	if err != nil {
		t.Fatalf("Failed to load hdfdata config: %v", err)
	}
...
func TestDefaultConfigHomeDirWithEnvAndFlag(t *testing.T) {
	...
	os.Setenv("HDFDATA_APPDATA_DIR", cliOverride)
	defer os.Unsetenv("HDFDATA_APPDATA_DIR")
...
func TestDefaultConfigTestNetWithEnv(t *testing.T) {
	os.Setenv("HDFDATA_USE_TESTNET", "true")
	defer os.Unsetenv("HDFDATA_USE_TESTNET")
...
func TestDefaultConfigTestNetWithEnvAndBadValue(t *testing.T) {
	os.Setenv("HDFDATA_USE_TESTNET", "no")
	defer os.Unsetenv("HDFDATA_USE_TESTNET")

	_, err := loadConfig()
	if err == nil {
		t.Errorf("Invalid boolean value for HDFDATA_USE_TESTNET did not cause an error.")
	}
}
