...
RateServer is a central server for distributing Hdfchain exchange rate data to
any number of clients. It is particularly useful for HDFData instances behind a
...
```
cd path/to/hdfdata/dcrrates/rateserver
go build
./rateserver
```

See [HDFData's notes on Go](https://github.com/hdfchain/hdfdata#building-hdfdata-with-go-111)
...
### Using RateServer with HDFData

To point HDFData to RateServer, you must set HDFData's `ratemaster` and
