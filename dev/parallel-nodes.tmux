#!/bin/sh
#
# Script to setup parallel hdfd nodes with separate wallets.
...
cat > "${NODES_ROOT}/hdfctl.conf" <<EOF
rpcuser=${RPCUSER}
rpcpass=${RPCPASS}
notls=1
simnet=1
...
cp ${NODES_ROOT}/hdfd.conf ${NODES_ROOT}/alpha
cat >> ${NODES_ROOT}/alpha/hdfd.conf <<EOF
listen=127.0.0.1:19100
rpclisten=127.0.0.1:19101
miningaddr=${WALLET01_MININGADDR}
EOF

cp ${NODES_ROOT}/hdfctl.conf ${NODES_ROOT}/alpha
cat >> ${NODES_ROOT}/alpha/hdfctl.conf <<EOF
rpcserver=127.0.0.1:19101
EOF

cp ${NODES_ROOT}/hdfctl.conf ${NODES_ROOT}/w-alpha
cat >> ${NODES_ROOT}/w-alpha/hdfctl.conf <<EOF
walletrpcserver=127.0.0.1:19102
EOF


cp ${NODES_ROOT}/hdfd.conf ${NODES_ROOT}/beta
cat >> ${NODES_ROOT}/beta/hdfd.conf <<EOF
listen=127.0.0.1:19200
rpclisten=127.0.0.1:19201
miningaddr=${WALLET02_MININGADDR}
EOF

cp ${NODES_ROOT}/hdfctl.conf ${NODES_ROOT}/beta
cat >> ${NODES_ROOT}/beta/hdfctl.conf <<EOF
rpcserver=127.0.0.1:19201
EOF

cp ${NODES_ROOT}/hdfctl.conf ${NODES_ROOT}/w-beta
cat >> ${NODES_ROOT}/w-beta/hdfctl.conf <<EOF
...
for i in \$(seq \$NUM) ; do
  hdfctl -C ./hdfctl.conf generate 1
  sleep 0.3
done
EOF
...
#!/bin/sh
hdfctl -C ./hdfctl.conf --wallet -c ./data/rpc.cert \$*
EOF
...
tmux send-keys "hdfd -C ./hdfd.conf" C-m
...
tmux send-keys "hdfd -C ./hdfd.conf --connect 127.0.0.1:19100" C-m
tmux select-pane -t 1
tmux send-keys "cd beta" C-m
#sleep 3
#tmux send-keys "./ctl generate 16" C-m
...
tmux send-keys "hdfwallet -C ../wallet.conf --create" C-m
sleep 2
tmux send-keys "123" C-m "123" C-m "n" C-m "y" C-m
sleep 1
tmux send-keys "${WALLET01_SEED}" C-m C-m
tmux send-keys "hdfwallet -C ../wallet.conf --rpcconnect 127.0.0.1:19101 \
...
tmux send-keys "hdfwallet -C ../wallet.conf --rpcconnect 127.0.0.1:19201 \
