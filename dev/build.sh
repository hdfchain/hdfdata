#!/bin/bash
# Usage: build.sh [hdfdata_root] [destination_folder]
#
...
#   root folder. If not running from a git repository, the hdfdata_root folder
#   must be specified.
#
#   Specify destination_folder to install the hdfdata executable and the static
...
if [[ -z "$ROOT" ]]; then
    echo "Not in git repository. You must specify the hdfdata root folder as the first argument!"
    exit 1
fi
...
echo "Building the hdfdata binary..."
GO111MODULE=on go build -v -o hdfdata
...
if [[ -n "$DEST" ]]; then
    sudo install -m 754 -o hdfdata -g hdfchain ./hdfdata ${DEST}/
