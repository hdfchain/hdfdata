...
// it into the BlockDataBasic and StakeInfoExtended and hdfdataapi types.
...
func (t *Collector) CollectHash(hash *chainhash.Hash) (*BlockData, *wire.MsgBlock, error) {
	...
	// still running, or hdfd may be mad.
	t.mtx.Lock()
	defer t.mtx.Unlock()
...
func (t *Collector) Collect() (*BlockData, *wire.MsgBlock, error) {
	...
	case <-time.After(time.Second * 10):
		log.Errorf("Timeout waiting for hdfd.")
