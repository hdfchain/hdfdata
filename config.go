...
import (
	...
	"github.com/hdfchain/hdfdata/v5/netparams"
	"github.com/hdfchain/hdfdata/v5/version"
	"github.com/decred/slog"
	flags "github.com/jessevdk/go-flags"
)

const (
	defaultConfigFilename = "hdfdata.conf"
	defaultLogFilename    = "hdfdata.log"
...
var (
	defaultHomeDir           = dcrutil.AppDataDir("hdfdata", false)
	...
	defaultPoliteiaAPIURl    = "https://proposals.clkj.ltd"
	defaultChartsCacheDump   = "chartscache.gob"

	defaultPGHost           = "127.0.0.1:5432"
	defaultPGUser           = "hdfdata"
	defaultPGPass           = ""
	defaultPGDBName         = "hdfdata"
	...
	defaultMainnetLink  = "https://explorer.hdfdata.org/"
	defaultTestnetLink  = "https://testnet.hdfdata.org/"
	defaultOnionAddress = ""
...
type config struct {
	...
	HomeDir      string `short:"A" long:"appdata" description:"Path to application home directory" env:"HDFDATA_APPDATA_DIR"`
	ConfigFile   string `short:"C" long:"configfile" description:"Path to configuration file" env:"HDFDATA_CONFIG_FILE"`
	DataDir      string `short:"b" long:"datadir" description:"Directory to store data" env:"HDFDATA_DATA_DIR"`
	LogDir       string `long:"logdir" description:"Directory to log output." env:"HDFDATA_LOG_DIR"`
	...
	OutFolder    string `short:"f" long:"outfolder" description:"Folder for file outputs" env:"HDFDATA_OUT_FOLDER"`
	ShowVersion  bool   `short:"V" long:"version" description:"Display version information and exit"`
	TestNet      bool   `long:"testnet" description:"Use the test network (default mainnet)" env:"HDFDATA_USE_TESTNET"`
	SimNet       bool   `long:"simnet" description:"Use the simulation test network (default mainnet)" env:"HDFDATA_USE_SIMNET"`
	DebugLevel   string `short:"d" long:"debuglevel" description:"Logging level {trace, debug, info, warn, error, critical}" env:"HDFDATA_LOG_LEVEL"`
	Quiet        bool   `short:"q" long:"quiet" description:"Easy way to set debuglevel to error" env:"HDFDATA_QUIET"`
	HTTPProfile  bool   `long:"httpprof" short:"p" description:"Start HTTP profiler." env:"HDFDATA_ENABLE_HTTP_PROFILER"`
	HTTPProfPath string `long:"httpprofprefix" description:"URL path prefix for the HTTP profiler." env:"HDFDATA_HTTP_PROFILER_PREFIX"`
	CPUProfile   string `long:"cpuprofile" description:"File for CPU profiling." env:"HDFDATA_CPU_PROFILER_FILE"`
	UseGops      bool   `short:"g" long:"gops" description:"Run with gops diagnostics agent listening. See github.com/google/gops for more information." env:"HDFDATA_USE_GOPS"`
	ReloadHTML   bool   `long:"reload-html" description:"Reload HTML templates on every request" env:"HDFDATA_RELOAD_HTML"`

	// API/server
	APIProto            string  `long:"apiproto" description:"Protocol for API (http or https)" env:"HDFDATA_ENABLE_HTTPS"`
	APIListen           string  `long:"apilisten" description:"Listen address for API. default localhost:7777, :17778 testnet, :17779 simnet" env:"HDFDATA_LISTEN_URL"`
	...
	UseRealIP           bool    `long:"userealip" description:"Use the RealIP middleware from the pressly/chi/middleware package to get the client's real IP from the X-Forwarded-For or X-Real-IP headers, in that order." env:"HDFDATA_USE_REAL_IP"`
	CacheControlMaxAge  int     `long:"cachecontrol-maxage" description:"Set CacheControl in the HTTP response header to a value in seconds for clients to cache the response. This applies only to FileServer routes." env:"HDFDATA_MAX_CACHE_AGE"`
	InsightReqRateLimit float64 `long:"insight-limit-rps" description:"Requests/second per client IP for the Insight API's rate limiter." env:"HDFDATA_INSIGHT_RATE_LIMIT"`
	...
	// Mempool
	MempoolMinInterval int `long:"mp-min-interval" description:"The minimum time in seconds between mempool reports, regardless of number of new tickets seen." env:"HDFDATA_MEMPOOL_MIN_INTERVAL"`
	MempoolMaxInterval int `long:"mp-max-interval" description:"The maximum time in seconds between mempool reports (within a couple seconds), regardless of number of new tickets seen." env:"HDFDATA_MEMPOOL_MAX_INTERVAL"`
	MPTriggerTickets   int `long:"mp-ticket-trigger" description:"The minimum number of new tickets that must be seen to trigger a new mempool report." env:"HDFDATA_MP_TRIGGER_TICKETS"`

	// Politeia/proposals and consensus agendas
	AgendasDBFileName string `long:"agendadbfile" description:"Agendas DB file name (default is agendas.db)." env:"HDFDATA_AGENDAS_DB_FILE_NAME"`
	ProposalsFileName string `long:"proposalsdbfile" description:"Proposals DB file name (default is proposals.db)." env:"HDFDATA_PROPOSALS_DB_FILE_NAME"`
	PoliteiaAPIURL    string `long:"politeiaurl" description:"Defines the root API politeia URL (defaults to https://proposals.clkj.ltd)."`
	...
	NoDevPrefetch    bool   `long:"no-dev-prefetch" description:"Disable automatic dev fund balance query on new blocks. When true, the query will still be run on demand, but not automatically after new blocks are connected." env:"HDFDATA_DISABLE_DEV_PREFETCH"`
	...
	// DB backend
	PGDBName         string        `long:"pgdbname" description:"PostgreSQL DB name." env:"HDFDATA_PG_DB_NAME"`
	PGUser           string        `long:"pguser" description:"PostgreSQL DB user." env:"HDFDATA_POSTGRES_USER"`
	PGPass           string        `long:"pgpass" description:"PostgreSQL DB password." env:"HDFDATA_POSTGRES_PASS"`
	PGHost           string        `long:"pghost" description:"PostgreSQL server host:port or UNIX socket (e.g. /run/postgresql)." env:"HDFDATA_POSTGRES_HOST_URL"`
	...
	SyncAndQuit      bool          `long:"sync-and-quit" description:"Sync to the best block and exit. Do not start the explorer or API." env:"HDFDATA_ENABLE_SYNC_N_QUIT"`
	ImportSideChains bool          `long:"import-side-chains" description:"(experimental) Enable startup import of side chains retrieved from hdfd via getchaintips." env:"HDFDATA_IMPORT_SIDE_CHAINS"`
	...
	// RPC client options
	HdfdUser         string `long:"hdfduser" description:"Daemon RPC user name" env:"HDFDATA_HDFD_USER"`
	HdfdPass         string `long:"hdfdpass" description:"Daemon RPC password" env:"HDFDATA_HDFD_PASS"`
	Hdfdserv         string `long:"hdfdserv" description:"Hostname/IP and port of hdfd RPC server to connect to (default localhost:9109, testnet: localhost:19109, simnet: localhost:19556)" env:"HDFDATA_HDFD_URL"`
	HdfdCert         string `long:"hdfdcert" description:"File containing the hdfd certificate file" env:"HDFDATA_HDFD_CERT"`
	DisableDaemonTLS bool   `long:"nodaemontls" description:"Disable TLS for the daemon RPC client -- NOTE: This is only allowed if the RPC client is connecting to localhost" env:"HDFDATA_HDFD_DISABLE_TLS"`
	NoBlockPrefetch  bool   `long:"no-hdfd-block-prefetch" description:"Disable block pre-fetch from hdfd during startup sync."`

	// ExchangeBot settings
	EnableExchangeBot bool   `long:"exchange-monitor" description:"Enable the exchange monitor" env:"HDFDATA_MONITOR_EXCHANGES"`
	DisabledExchanges string `long:"disable-exchange" description:"Exchanges to disable. See /exchanges/exchanges.go for available exchanges. Use a comma to separate multiple exchanges" env:"HDFDATA_DISABLE_EXCHANGES"`
	ExchangeCurrency  string `long:"exchange-currency" description:"The default bitcoin price index. A 3-letter currency code" env:"HDFDATA_EXCHANGE_INDEX"`
	RateMaster        string `long:"ratemaster" description:"The address of a DCRRates instance. Exchange monitoring will get all data from a DCRRates subscription." env:"HDFDATA_RATE_MASTER"`
	RateCertificate   string `long:"ratecert" description:"File containing DCRRates TLS certificate file." env:"HDFDATA_RATE_MASTER"`

	// Links
	MainnetLink  string `long:"mainnet-link" description:"When hdfdata is on testnet, this address will be used to direct a user to a hdfdata on mainnet when appropriate." env:"HDFDATA_MAINNET_LINK"`
	TestnetLink  string `long:"testnet-link" description:"When hdfdata is on mainnet, this address will be used to direct a user to a hdfdata on testnet when appropriate." env:"HDFDATA_TESTNET_LINK"`
	OnionAddress string `long:"onion-address" description:"Hidden service address" env:"HDFDATA_ONION_ADDRESS"`
}
...
var (
	defaultConfig = config{
		...
		HdfdCert:            defaultDaemonRPCCertFile,
...
func loadConfig() (*config, error) {
	...
	// them.
	cfg.Hdfdserv, err = normalizeNetworkAddress(cfg.Hdfdserv, defaultHost, activeNet.JSONRPCClientPort)
	if err != nil {
		return loadConfigError(err)
	}
	...
	cfg.HdfdCert = cleanAndExpandPath(cfg.HdfdCert)
...
// netName returns the name used when referring to a hdfchain network. TestNet3
...
func netName(chainParams *netparams.Params) string {
	...
	// TestNet2 was removed entirely for hdfd 1.3.0. Compile check!
