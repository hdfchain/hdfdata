...
// much of the time, hdfdata will be using the types in chainjson, but others
// are defined here
...
// of this type are NOT compatible with hdfd's txscript.ScriptClass values! Use
...
// class. If hdfd's txscript package changes its strings, this function may be
// unable to identify the types from hdfd.
...
// TxOut defines a hdfchain transaction output.
type TxOut struct {
	Value               float64      `json:"value"`
	Version             uint16       `json:"version"`
	ScriptPubKeyDecoded ScriptPubKey `json:"scriptPubKey"`
...
// TxIn defines a hdfchain transaction input.
type TxIn struct {
	// Non-witness
...
type APIStatus struct {
	...
	DcrdataVersion  string `json:"hdfdata_version"`
	NetworkName     string `json:"network_name"`
}

// NewStatus is the constructor for a new Status.
func NewStatus(nodeHeight uint32, conns int64, apiVersion int, hdfdataVersion, netName string) *Status {
	return &Status{
		height:          nodeHeight,
		nodeConnections: conns,
		api: APIStatus{
			APIVersion:     apiVersion,
			DcrdataVersion: hdfdataVersion,
			NetworkName:    netName,
		},
	}
}
...
// Happy describes just how happy hdfdata is.
type Happy struct {
...
// Happy indicates how hdfdata is or isn't happy.
