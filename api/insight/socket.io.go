...
func NewSocketServer(params *chaincfg.Params, txGetter txhelpers.RawTransactionGetter) (*SocketServer, error) {
	...
	// (currently just "inv" or a valid Hdfchain address), joins the room, and
	...
	server.OnEvent("/", "subscribe", func(so socketio.Conn, room string) string {
		...
		// See if the room is a Hdfchain address.
...
// SendNewTx prepares a hdfd mempool tx for broadcast. This method satisfies
