...
const (
	...
	// getrawtransaction RPC call to hdfd.
...
func (iapi *InsightApi) getAddressInfo(w http.ResponseWriter, r *http.Request) {
	...
	SPENDING_TX_DUPLICATE_CHECK:
		for _, f := range addressOuts.PrevOuts {
			...
			// because vin:i valuein is not reliable from hdfd at present.
			// TODO(chappjc): hdfd should be OK now. Recheck this.
...
func (iapi *InsightApi) getEstimateFee(w http.ResponseWriter, r *http.Request) {
	...
	// A better solution would be a call to the hdfd RPC "estimatefee" endpoint
